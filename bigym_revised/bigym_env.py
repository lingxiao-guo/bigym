...
from bigym.robots.configs.h1 import H1, H1_HIGH_GAIN
...
class BiGymEnv(gym.Env):
    ...
    def __init__(
        ...
        if high_gain:
            self.DEFAULT_ROBOT = H1_HIGH_GAIN
            print("Using High Gain robotiq xml...")
        else:
            print("Using origin robotiq xml...")
    ...
    def _get_proprioception_obs(self) -> dict[str, Any]:
        ...
        obs["proprioception_grippers"] = np.array(self.robot.qpos_grippers).astype(
                np.float32
            )
        return obs
